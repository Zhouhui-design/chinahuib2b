...
type DisclaimerTickerProps = {
  locale?: LanguageCode
}

export default function DisclaimerTicker({ locale = 'en' }: DisclaimerTickerProps) {
  const disclaimers = {
    ...
    es: '⚠️ Aviso importante: Esta plataforma solo proporciona servicios de exhibición de información y emparejamiento, no garantiza transacciones | Se recomienda utilizar métodos de transacción seguros como cartas de crédito y garantías de terceros | La plataforma no asume responsabilidad por disputas de transacciones | Elija socios de transacción con precaución',
    fr: '⚠️ Avis important : Cette plateforme fournit uniquement des services d\'affichage d\'informations et de mise en relation, ne garantit pas les transactions | Il est recommandé d\'utiliser des méthodes de transaction sécurisées telles que les lettres de crédit et les garanties tierces | La plateforme n\'assume aucune responsabilité pour les litiges de transaction | Choisissez vos partenaires de transaction avec prudence',
    ...
    ar: '⚠️ تنبيه مهم: تقدم هذه المنصة خدمات عرض المعلومات والمطابقة فقط، ولا تقدم ضمانات للمعاملات | نوصي باستخدام طرق معاملات آمنة مثل خطابات الاعتماد والضمانات من طرف ثالث | المنصة لا تتحمل أي مسؤولية عن نزاعات المعاملات | يرجى اختيار شركاء المعاملات بحذر',
    ...
    pt: '⚠️ Aviso importante: Esta plataforma fornece apenas serviços de exibição de informações e correspondência, não garante transações | Recomenda-se usar métodos de transação seguros, como cartas de crédito e garantias de terceiros | A plataforma não assume responsabilidade por disputas de transações | Escolha parceiros de transação com cuidado',
